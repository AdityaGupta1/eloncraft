...
public class BlockLOXCollector extends BlockMachine {
    public BlockLOXCollector(String name, Material material) {
        super(name, material);
    }
    ...
    public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random random) {
        if (state.getValue(IS_ON)) {
            for (int i = 0; i < 3; i++) {
                world.spawnParticle(EnumParticleTypes.SNOWBALL, pos.getX() + 0.5 + randomCoordinateAdd(random),
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        TileEntityLOXCollector tileEntity = (TileEntityLOXCollector) world.getTileEntity(pos);
        ...
        if (StackUtil.isValid(heldItem) && FluidUtil.getFluidHandler(heldItem) != null) {
            ...
            player.openGui(Eloncraft.INSTANCE, GuiReference.LOX_COLLECTOR, world, pos.getX(), pos.getY(), pos.getZ());
        }
