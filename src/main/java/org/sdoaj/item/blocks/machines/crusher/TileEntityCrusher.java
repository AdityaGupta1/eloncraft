...
package org.sdoaj.item.blocks.machines.crusher;

import net.minecraft.item.ItemStack;
import org.sdoaj.item.blocks.machines.BlockMachine;
import org.sdoaj.item.blocks.machines.TileEntityInventoryMachine;
import org.sdoaj.item.blocks.tileentities.CustomEnergyStorage;
...
public class TileEntityCrusher extends TileEntityInventoryMachine {
    ...
    public boolean canProcess() {
        if (StackUtil.isValid(this.inventory.getStackInSlot(SLOT_INPUT))) {
            CrusherRecipe recipe = CrusherRecipes.getRecipeFromInput(this.inventory.getStackInSlot(SLOT_INPUT));
            if (recipe == null) {
                return false;
            }
